...
export async function GET(request: NextRequest) {
  try {
    ...
    gradient.addColorStop(0, 'rgba(34, 197, 94, 0.12)');  // Green tint at top
    ...
    ctx.fillText('Good news! No problematic accounts found', width / 2, 200);

    // Add shadcn-style UI card
    drawCard(ctx, width / 2 - 300, 240, 600, 280);
    
    // Draw checkmark and success message
    drawSuccessSymbol(ctx, width / 2, 380, 80);
...
function drawModernGrid(ctx: import('canvas').CanvasRenderingContext2D, width: number, height: number) {
  ...
  ctx.fillStyle = 'rgba(34, 197, 94, 0.15)'; // Green accent
  ctx.beginPath();
...
/**
 * Draws a success checkmark with message
 */
function drawSuccessSymbol(ctx: import('canvas').CanvasRenderingContext2D, x: number, y: number, size: number) {
  // Large checkmark
  ...
  ctx.fillText('Your following list looks good', x, y + size + 65);
}
...
function drawLogo(ctx: import('canvas').CanvasRenderingContext2D, x: number, y: number, size: number) {
  ...
  // Logo symbol - shield with checkmark
  ctx.beginPath();
