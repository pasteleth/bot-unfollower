...
/**
 * Generate a shadcn-style scanning image for the frame
 */
export async function GET(request: NextRequest) {
  try {
    ...
    gradient.addColorStop(0, 'rgba(14, 165, 233, 0.12)');  // Sky blue tint at top
    ...
    ctx.fillText('Scanning In Progress', width / 2, 140);

    // Description
    ctx.font = `28px ${bodyFont}`;
    ctx.fillStyle = '#a1a1aa'; // Muted gray
    ctx.fillText('Checking accounts in your following list...', width / 2, 200);

    // Add shadcn-style UI card
    drawCard(ctx, width / 2 - 300, 240, 600, 280);
    
    // Draw loading animation
    drawLoadingAnimation(ctx, width / 2, 380, 80);
...
function drawModernGrid(ctx: import('canvas').CanvasRenderingContext2D, width: number, height: number) {
  ...
  ctx.fillStyle = 'rgba(14, 165, 233, 0.15)'; // Sky blue accent
  ctx.beginPath();
 ...
 */
function drawLoadingAnimation(ctx: import('canvas').CanvasRenderingContext2D, x: number, y: number, size: number) {
  // Draw a progress bar
  ...
  ctx.beginPath();
  ctx.roundRect(x - barWidth / 2, y - barHeight / 2, barWidth, barHeight, barHeight / 2);
  ctx.fill();
  
  // Progress indicator
  ctx.fillStyle = '#0ea5e9'; // Sky blue
  const progressWidth = barWidth * 0.6; // 60% complete
  ctx.beginPath();
  ctx.roundRect(x - barWidth / 2, y - barHeight / 2, progressWidth, barHeight, barHeight / 2);
  ctx.fill();
  
  // Draw scanning indicators
  drawScanLines(ctx, x, y - barHeight * 4, barWidth, barHeight * 3);
  ...
  ctx.textAlign = 'center';
  ctx.fillText('Scanning...', x, y + size / 2);
  
  // Progress percentage
  ctx.font = `18px ${titleFont}`;
  ctx.fillStyle = '#ffffff';
  ctx.fillText('60%', x, y + 5);
  
  // Additional message
  ctx.font = `20px ${titleFont}`;
  ctx.fillStyle = '#a1a1aa';
  ctx.fillText('Results will be ready soon', x, y + size / 2 + 40);
}

/**
 * Draw scan lines effect
 */
function drawScanLines(ctx: import('canvas').CanvasRenderingContext2D, x: number, y: number, width: number, height: number) {
  ctx.save();
  ...
  for (let i = 0; i < 8; i++) {
    const yPos = y + (i * height / 8);
    const lineAlpha = i % 2 === 0 ? 0.6 : 0.3;
  ...
  for (let i = 0; i < 15; i++) {
    const xPos = x - width / 2 + (i * width / 15);
    const lineAlpha = i % 3 === 0 ? 0.5 : 0.2;
    ctx.strokeStyle = `rgba(14, 165, 233, ${lineAlpha})`;
  ...
  const scanLineY = y + height * 0.6;
  ctx.strokeStyle = 'rgba(14, 165, 233, 0.9)';
  ctx.lineWidth = 2;
  
  ctx.beginPath();
  ctx.moveTo(x - width / 2, scanLineY);
  ctx.lineTo(x + width / 2, scanLineY);
  ctx.stroke();
...
function drawLogo(ctx: import('canvas').CanvasRenderingContext2D, x: number, y: number, size: number) {
  ...
  // Logo symbol - shield with scanning line
  ctx.beginPath();
